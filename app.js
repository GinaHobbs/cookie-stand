function makeShop(location, averageCookiesPurchased, minCustomers, maxCustomers, businessOpen, businessClose) {
  var shop = {
    location: location,
    averageCookiesPurchased: averageCookiesPurchased,
    customers: 0,
    ...
    businessHours: 0,
    init: function(){   
    this.getCustomers(shop.minCustomers, shop.maxCustomers);
    this.getBusinessHours(businessOpen,businessClose);
...
let shopArray = [];
shopArray.push(new makeShop("Seattle", 6.3, 23, 65));
makeShop.init();
//{
...
const divElem = document.getElementById('list')
  ...
  h2Elem.textContent = shopArray[0].location;
  ...
  for (let i = 0; i < shopArray[0].cookiesPerDay.length; i++) {
    const liElem = document.createElement('li');
    liElem.textContent = shopArray[0].cookiesPerDay[i];
    ulElem.appendChild(liElem);
  }
  const liElem = document.createElement('li');
  liElem.textContent = 'Total Cookies: ' + shopArray[0].totalCookies;
  ulElem.appendChild(liElem);
